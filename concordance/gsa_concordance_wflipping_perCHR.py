...
def get_aggregate_R2(geno_df, imp_df, maf_bins=None):
    ...
    for i in range(1, len(maf_bins)):
        ...
        true_values.append(truth.tolist())
        imputed_values.append(imptd.tolist())
...
maf_df = pd.read_csv(maf_filename,header=None,sep=" ")
...
###### for emphasizing rarer variants                                                                                     
...
r2_2, nsnps_2, maf_2, true_gt_list2, impted_gt_list2 = get_aggregate_R2(gsa_fixed,imputed_ds_fixed,maf_bins2)
aggregation_df2 = pd.DataFrame(list(zip(maf_2, nsnps_2, r2_2)), columns=['maf_category', 'number_of_snps', 'R2'])

gt_lists2 = pd.DataFrame(list(zip(maf_2,true_gt_list2, impted_gt_list2)), columns=['MAF', 'True_list','\
Imputed_list'])
...
gt_list_name2 = cohort_name + "_rare_genotypes_per_MAFbin_table.tsv"
gt_lists2.to_csv(gt_list_name2,index=False,sep="\t")
